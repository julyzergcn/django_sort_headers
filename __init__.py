'''
from sort_headers import SortHeaders
...
def user_list(request):
    ...
    return render_to_response('users/user_list.html', {
        'users': users,
        'headers': sort_headers.get_headers(),
    })
'''
...
class SortHeaders(object):
    ...
    def __init__(self, request, headers, default_order_field=None,
            default_order_type='desc', additional_params=None):
        """
        request
    ...
    def headers(self):
        ...
        for i, (header, order_criterion) in enumerate(self.header_defs):
            ...
            if i == self.order_field:
                th_classes.append('%s' % self.order_type)
                new_order_type = {'asc': 'desc', 'desc': 'asc'}[self.order_type]
                arrow = self.order_type=='asc' and ' &uarr;' or ' &darr;'
            yield {
                'text': header + arrow,
    ...
    def get_headers(self):
        return [i for i in self.headers()]
